...
void Display_init(void) {
	const uint16_t InitData[] ={
		...
		//pause
		0xEF08,	0x1805,	//0x1805 Quer
        0x1283, 0x1500,	0x1300, 0x16AF 	//Querformat 176 x 132 Pixel
	};
	Waitms(300);
...
int main(void){
    ...
    //Beginn Ihres Programmteils

    for(uint16_t i = 0; i < 132*176; i++){
        SPISend8Bit(0x07); //send green
        SPISend8Bit(0xE0);
    }
    ...
    for(uint16_t i = 0; i < 140*76; i++){
        SPISend8Bit(0xF8);
        SPISend8Bit(0x00);//send red
    }
    */
    
    static volatile uint16_t lastColour;
    static volatile uint16_t currentColour;
    static volatile uint16_t possibleRepitions;
    
    for(uint16_t i = 0; i < 2904; i++){
        lastColour = Bild1[i];
        currentColour = Bild1[i+1];
        possibleRepitions = Bild1[i+2];
        
        if(lastColour == currentColour) {
            SPISend16Bit(lastColour);
            for(uint16_t j = 0; j < possibleRepitions + 1; j++) { //wenn Anzahl an Wiederholungen, dann so oft nochmal senden //+1 damit falls 0 nochmal druckt
                SPISend16Bit(lastColour);
            }
            i += 2;
        } else {
            SPISend16Bit(lastColour); //Farbe einmalig senden
        }
    }
